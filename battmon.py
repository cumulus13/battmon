    ...
    try:
        while 1:
            ...
            if int(battery.percent) == 100:
                #try:
                notify.send('Battery is FULL', 'Battery is FULL', 'battmon', 'FULL', icon = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'icon.png'))
                #except:
                ...
                #try:
                notify.send('Battery is LOW', 'Battery is LOW', 'battmon', 'LOW', icon = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'icon.png'))
                #except:
            ...
            if int(battery.percent) in [11, 20, 30, 40, 50, 60, 70, 80, 90, 99]:
                if config.get_config('remind', 'ten'):
                    ...
                    notify.send('Battery STATUS', message, 'battmon', 'STATUS', icon = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'icon.png'))
            try:
                print(make_colors(datetime.strftime(datetime.now(), '%Y/%m/%d %H:%M:%S:%f'), 'lb') + " " + "-"*(cmdw.getWidth() - 30))
            except:
                print(make_colors(datetime.strftime(datetime.now(), '%Y/%m/%d %H:%M:%S:%f'), 'lb') + " ------- ")
